%BARUMERLI2022_COORDINATES Class useful to handl HRTF coordinates with different conventions
%   Usage: coordinates = barumerli2022_coordinates(SOFAobj);
%
%   Interface:
% 
%       barumerli2022_coordinates: constructor. The coordinates are converted into the 
...
%   ---------
% 
%   coordinates = barumerli2022_coordinates(SOFAobj);
...
%
%   Url: http://amtoolbox.org/amt-1.2.0/doc/modelstages/barumerli2022_coordinates.php
...
classdef barumerli2022_coordinates < handle
    ...
    methods
        % constructor
        function obj = barumerli2022_coordinates(data, convention)
            if nargin == 1
                if strcmp(data.GLOBAL_Conventions, 'SOFA')
                    obj.pos = SOFAcalculateAPV(data);
                    obj.pos_type = data.SourcePosition_Type;
                else
                    error('barumerli2022_coordinates: SOFA object not valid!');
                end
            elseif nargin == 2
                if ismatrix(data)
                    ...
                    error('barumerli2022_coordinates: position matrix not valid!');
                end
            else
                obj.pos = zeros(0,3);
...
%       r: actual radius
    ...
    coords_new = barumerli2022_coordinates(coords.pos(idx,:), coords.pos_type);
end
